...
const getPreviousAndNextPokemon = (currentId) => {
    const previousId = currentId > 1 ? currentId - 1 : 1025;
...
const updateLangSwitch = (language) => {
    ...
    const langSwitchFlag = document.getElementById("current-flag");
    if (language === 'english') {
        langSwitch.classList.add("active");
        langSwitchFlag.className = 'flag fi-us';
        langSwitch.title = "English";
    } else {
        langSwitch.classList.remove("active");
        langSwitchFlag.className = 'flag fi-ve';
...
const getMissingno = () => {
    pokemon = {
        ...
        "speciesData": {
            "description": "MissingNo is a glitched Pokémon that is often associated with the game's programming errors. It is known for its unpredictable behavior and unique appearance.",
            "types": ["normal", "glitch"],
            "stats": {
            "hp": 60,
            "attack": 90,
